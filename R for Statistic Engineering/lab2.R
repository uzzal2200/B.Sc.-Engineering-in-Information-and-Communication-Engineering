...
cat("Mean:", mean(chi2_data), "\nVariance:", var(chi2_data), "\n")

par(mfrow = c(1, 3))

hist(chi2_data, breaks = 30, col = "skyblue", main = "Chi-Squared Distribution", xlab = "Value", border = "white")
lines(density(chi2_data), col = "red", lwd = 2)

plot(density(chi2_data), col = "blue", lwd = 2, main = "Density Plot", xlab = "Value")
curve(dchisq(x, df = k), col = "red", lwd = 2)
